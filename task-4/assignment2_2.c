...
int main() {
    ...
    printf("%-.2f %c %-.2f = %-.2f \n",operand1,operation_char, operand2, result);

    printf("%d\n",sizeof(operation));
        /*If we add extra members to the enum, the size of the enum will depend on the values of the new members.
        If the values of the new members are within the range of the underlying type, the size of the enum may not change.
        However, if the values of the new members exceed the range of the underlying type,
        the size of the enum may increase to accommodate the larger values.*/
    return 0;
}
