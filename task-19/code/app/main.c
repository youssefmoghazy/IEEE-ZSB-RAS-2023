...
{
    LCD_voidClear();
    LCD_voidSendString("1DCMotor 3Exit");
    LCD_u8GoToXY(LCD_u8_Line2, 0);
    LCD_voidSendString("2Stepper 4Servo");
}
...
void takeUserOption(u8 *option)
{
    while(1)
    {
        ...
        if(*option == DC_MOTOR_OPTION_CW || *option == DC_MOTOR_OPTION_CCW
        	|| *option == STEPPER_MOTOR_OPTION_CW || *option == STEPPER_MOTOR_OPTION_CCW || *option == EXIT_OPTION||*option == SERVO)
        {
        	break;
        }
    }
